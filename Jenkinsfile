        ...
        stage('Stop java on prod') {
            steps {
                sshagent(credentials : ['58813893-d334-4f40-9a70-196b5fe89664']) {
                   sh 'ssh -o StrictHostKeyChecking=no jenkins@$PROD1 "echo $HOSTNAME && echo $ID && systemctl stop app.service && sleep 5s"'
                   sh 'ssh -o StrictHostKeyChecking=no jenkins@$PROD2 "echo $HOSTNAME && echo $ID && systemctl stop app.service && sleep 5s"'
                }
            }
        }
